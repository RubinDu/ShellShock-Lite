...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    if(code != 0) {
    	xil_printf("There is an error! MAXreg_wr%d", code);
    }
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    //if return code != 0 print an error
    if(code != 0) {
    	xil_printf("There is an error! MAXbytes_wr%d", code);
    }
    //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
    XSpi_SetSlaveSelect(&SpiInstance, 0x00000000);
    //return (data + nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    //read return code from SPI peripheral
    if(code != 0) {
        xil_printf("There is an error MAXreg_rd%d", code);
    }
    //if return code != 0 print an error
    //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
    XSpi_SetSlaveSelect(&SpiInstance, 0x00000000);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    //if return code != 0 print an error
    if(code != 0) {
    	xil_printf("There is an error! MAXbytes_rd%d", code);
    }
    for(BYTE i=1; i<nbytes+1; i++){
